...
func run(c *http, events ...common.MapStr) (string, error) {
...
func TestConsoleOneEvent(t *testing.T) {
	lines, err := run(newHttp(false), event("event", "myevent"))
	assert.Nil(t, err)
...
func TestConsoleOneEventIndented(t *testing.T) {
	lines, err := run(newHttp(true), event("event", "myevent"))
	assert.Nil(t, err)
	expected := "{\n  \"event\": \"myevent\"\n}\n"
	assert.Equal(t, expected, lines)
}
